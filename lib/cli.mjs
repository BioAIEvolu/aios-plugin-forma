...
// The bundle ships its own patch that inserts `{ id: 'forma' }` at install
// time; profile layers apply AFTER every bundle layer. Writing our own
// insert row therefore duplicates the entry id and Cordis refuses to boot
// ("duplicate loader entry id: forma"). The CLI must instead append an
// override patch — a bare `{ id, name, config }` layer with no `insert` —
// which applyEntryPatches applies to the bundle-provided entry in place.
function formaRow(config) { return { id: 'forma', name: PACKAGE_NAME, config }; }
function isFormaOverrideLayer(layer) { return layer != null && !Array.isArray(layer.insert) && layer.id === 'forma'; }
function findFormaRow(value) {
  if (!Array.isArray(value)) return null;
  let found = null;
  for (const layer of value) {
    if (isFormaOverrideLayer(layer)) found = layer;
    for (const entry of (Array.isArray(layer?.insert) ? layer.insert : [])) if (entry?.id === 'forma') found = entry;
  }
  return found;
}
function findFormaOverride(value) {
  if (!Array.isArray(value)) return null;
  let found = null;
  for (const layer of value) if (isFormaOverrideLayer(layer)) found = layer;
  return found;
}
...
function removeForma(value) {
  return (Array.isArray(value) ? value : [])
    .map(layer => ({ ...layer, insert: Array.isArray(layer.insert) ? layer.insert.filter(entry => entry.id !== 'forma') : layer.insert }))
    .filter(layer => !isFormaOverrideLayer(layer))
    .filter(layer => layer.insert === undefined || (Array.isArray(layer.insert) && layer.insert.length > 0));
}
...
async function install(options, ctx) {
  ...
  try {
    ...
    // Only an override-shaped row proves a healthy prior install; a legacy
    // insert-shaped row (v0.2.0) breaks boot, so it falls through to rewrite.
    const existing = findFormaOverride(before.value);
...
async function inspect(options, ctx) {
  ...
  const version = (await readInstalledVersion(dir)) ?? (await readInstallRecord(dir))?.version ?? null;
  const declaredTools = await readDeclaredToolCount(dir);
