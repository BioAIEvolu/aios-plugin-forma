...
const ERROR_CATALOG = {
  ...
  PNPM_REQUIRED: { exit: 10, message: '未找到 pnpm，DSH 无法安装插件依赖。', action: '尝试 corepack enable 后用 corepack 准备 pnpm，或先用 npm 全局安装 pnpm；检查命令：pnpm --version。' },
...
function asCliError(error) {
  ...
  if (code === 'UNKNOWN_ERROR') cli.userMessage = '发生未分类错误。';
...
// ---------------------------------------------------------------------------
// DSH invocation (real implementation; tests inject a fake).
// ---------------------------------------------------------------------------
...
function realRunDsh(home, args, cwd) {
...
async function readToolCount(dir) {
...
async function install(options, ctx) {
  ...
  try {
    ...
    const toolCount = await readToolCount(dir);
    if (existing && sameConfig(existing.config, config) && installedBefore === packageJson.version) {
      ...
      result = { status: 'already-installed', version: installedBefore, toolCount, recordPath: await pathExists(recordPath) ? recordPath : null };
    } else {
      ...
      let dshResult;
      try {
        dshResult = await ctx.runDsh(home, ['plugin', '--profile', options.profile, 'install', packageSpec, '--config.ignore-scripts=true', '--yes'], home);
        ctx.verbose(`[dsh install stdout]\n${dshResult.stdout.trim()}\n[dsh install stderr]\n${dshResult.stderr.trim()}`);
        const dump = await ctx.runDsh(home, ['--profile', options.profile, '--dump-config'], home);
        ctx.verbose(`[dsh dump-config stdout]\n${dump.stdout.trim()}\n[dsh dump-config stderr]\n${dump.stderr.trim()}`);
      } catch (error) {
        let rolledBack = false;
        try { await ctx.runDsh(home, ['plugin', '--profile', options.profile, 'remove', PACKAGE_NAME, '--yes'], home); } catch { /* best effort rollback */ }
      ...
      const record = { schema_version: 1, package_name: PACKAGE_NAME, version, requested_url: download?.requested_url ?? null, final_url: redactUrl(download?.final_url), sha256: download?.sha256 ?? null, bytes: download?.bytes ?? null, result: { status: 'installed', dsh_exit_code: dshResult.code, dsh_output: scrub(dshResult.stdout).trim() }, recorded_at: new Date().toISOString() };
      const recordPath = join(dir, 'forma-install-record.json');
      await writeFile(recordPath, JSON.stringify(record, null, 2) + '\n');
      result = { status: existing ? 'updated' : 'installed', version, toolCount: await readToolCount(dir), recordPath, installedBefore };
    }
  } finally {
    if (download) {
  ...
  const payload = {
    ...
    tool_count: result.toolCount, next_steps: nextSteps, uninstall_command: uninstallCommand,
  ...
  else if (result.status === 'updated') lines.push(`[OK] 已更新并重新安装到 DSH Profile：${options.profile}（${result.installedBefore ?? '未知'} -> ${result.version}）`);
  else lines.push(`[OK] 已安装到 DSH Profile：${options.profile}`);
  lines.push('[OK] 运行时摘要校验通过');
  if (result.toolCount !== null) lines.push(`[OK] Forma 工具已就绪：${result.toolCount} 个`);
  else lines.push('[WARN] 未能读取 Forma 工具数量，请在 DSH 中确认');
...
async function inspect(options, ctx) {
  ...
  if (!row && !bundle) {
    const payload = { schema_version: 1, command: 'inspect', status: 'not-installed', package: PACKAGE_NAME, version: null, profile: options.profile, dsh_home: home, runtime_digest: null, requested_url: null, final_url: null, sha256: null, bytes: null, tool_count: null, source_roots: null, last_install: null, install_command: installCommand, error: null };
  ...
  const digestMatches = row?.config?.runtimeDigest && row.config.runtimeDigest === currentDigest;
  let lastInstall = null;
  try {
    const record = JSON.parse(await readFile(join(dir, 'forma-install-record.json'), 'utf8'));
    lastInstall = { recorded_at: record.recorded_at ?? null, sha256: record.sha256 ?? null, version: record.version ?? null };
  } catch { /* no record: installed by an older CLI */ }
  ...
  const payload = {
    ...
    sha256: lastInstall?.sha256 ?? null, bytes: null, tool_count: toolCount, source_roots: sourceRoots,
    last_install: lastInstall, error: null,
  };
  const lines = [
    'Forma 状态：已安装',
    `插件版本：${version ?? '未知'}`,
    ...
    `DSH_HOME：${home}`,
    `运行时：${digestMatches ? '健康' : '摘要与当前 CLI 不一致（插件与 CLI 版本可能不同）'}`,
  ];
  if (toolCount !== null) lines.push(`Forma 工具：${toolCount} 个`);
...
async function uninstall(options, ctx) {
  ...
  const payload = { schema_version: 1, command: 'uninstall', status: null, package: PACKAGE_NAME, version: null, profile: options.profile, dsh_home: home, runtime_digest: null, requested_url: null, final_url: null, sha256: null, bytes: null, error: null };
  ...
  try {
    const removed = await ctx.runDsh(home, ['plugin', '--profile', options.profile, 'remove', PACKAGE_NAME, '--yes'], home).catch(error => {
...
export const __testing = { ERROR_CATALOG, parse, scanModes, shortSha, lastLine, sameConfig, findFormaRow };
